...
| 关联 | [PRD §4.5 / §4.3.7](../progress/requirements-mvp-v0.1.md)、[architecture §8.3–9](../architecture/architecture-design-spec.md)、[gateway-integration](../reference/gateway-integration.md)、ADR-001/002、[ADR-004](004-ai-seat-memory.md)、[ADR-006](006-mock-vs-llm-intent.md)（Mock/LLM 排查） |
...
| LLM | DeepSeek 官方 API；单局单模型；**6s** 超时 → Mock fallback；JSON 解析失败**最多重试 2 次**（非法 intent 不重试）；**不做 LLM 预取** |
...
| LLM 6s | 虚拟线程/异步调用；超时 → Mock；解析失败最多重试 2 次；**不**长时间占用房间写锁 |
