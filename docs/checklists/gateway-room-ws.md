...
> **定位**：本页为 B 侧**执行勾选清单**；架构与实现以 [ADR-005](../adr/005-gateway-formal-path.md) 为准。
...
| G-03 | 连接管理器 | 维护 `sessionId`、`roomId`、`playerId`、心跳状态 | 部分完成：内存 `ConnectionManager`；**心跳未实现** |
| G-04 | 消息路由器 | 按 `type` 分发到 room / game / chat 逻辑 | **已完成**：`MessageRouter` 接 game；`CHAT_MESSAGE`/`WOLF_CHAT` 经 `ChatMessageService` |
...
| R-02 | 加入房间 | 分配 `playerId`，支持 AI 预占位 | **已完成**：不传 `seatId` 时自动分座 |
| R-03 | 准备 / 取消准备 | 更新座位 ready 状态 | 已完成 |
| R-04 | 开始游戏 | 校验人数与 ready，触发 `ROLE_ASSIGN` | 已完成 |
| R-05 | 解散房间 | 房主、未开局时可解散 | **已完成**：`DELETE /api/room/{id}` |
| R-06 | 房间快照 | 查询当前座位、状态、阶段 | **已完成**：`GET` 含 `seats[]` |
...
| M-05 | `GAME_EVENT` | 死亡、放逐、公开事件广播 | **已完成**：`PerceptionLogEvents` → WS `GAME_EVENT` |
...
- WS 会话生命周期
- 定向推送（Formal 路径已由 `WsPushService` 承担）
- 断线重连
- 房间连接映射
...
- Formal 主动推送经 `WsPushService`；勿将 `ActionResult.phaseSyncs` 无 `seatId` 标注地广播给全房。
...
| ID | 项 | 说明 | 优先级 | 状态 |
|----|----|------|--------|------|
| N-01 | WS 错误稳定性 | 异常统一返回 `ERROR`，避免 1011 断线 | P0 | 部分完成 |
| N-02 | 定向同步协议 | 为 `PHASE_SYNC` 补接收者 seatId / targeted wrapper | P0 | 部分完成：请求-响应外层已有 seatId |
| N-03 | 主动推送 | `GAME_ACTION` 后按座位推送最新 `PHASE_SYNC` | P1 | **已完成** |
| N-04 | 自动分配座位 | `join` 不传 seatId 时自动找空座 | P1 | **已完成** |
| N-05 | 断线重连 | 30s 内恢复 `roomId + seatId` 绑定 | P1 | 未开始 |
| N-06 | Redis 映射 | 把内存连接表迁移/同步到 Redis key | P2 | 未开始 |
| N-07 | 鉴权绑定 | token -> userId -> seat 权限校验 | P2 | 未开始 |
