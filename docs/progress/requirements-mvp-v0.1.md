...
#### 1.5.1 自进化 Agent 需求（新增）

> 目标：让**每个 Agent**基于**本局自己的发言与行动结果**做赛后分析，生成可审计的优化建议，并在人工或规则审核后更新下一版 Prompt。

| 项 | 需求 |
|----|------|
| 输入 | 本座位本局 `action_log`、`thinking`、发言文本、关键 `PHASE_SYNC` 片段、最终胜负 |
| 分析粒度 | 以 **单座位 / 单局** 为最小单位，不能跨座位合并结论后直接回写 |
| 分析内容 | 发言是否符合角色信息、是否暴露身份、是否与局势一致、是否漏掉关键线索、是否存在过激/无效表达 |
| 输出 | 结构化复盘结果，至少包含 `seatId`、`roomId`、`modelId`、`promptVersion`、`selfReview`、`suggestions`、`nextPromptDelta` |
| 写回方式 | 只允许离线 Job 生成候选 Prompt 版本；**必须**经过审核或规则校验后，才可发布为下一版 Prompt |
| 运行时约束 | 在线对局期间只读当前 Prompt；**禁止** Agent 在局内自修改 Prompt、Memory 或规则 |
| 版本控制 | 每次优化必须产生新 `promptVersion`，保留前一版本以便回放、对比和回滚 |
| 审计要求 | 复盘结果与发布记录写入 `action_log` / 评测表，便于追踪某次 Prompt 变更对应的胜率变化 |
| 安全边界 | 不得把其他玩家私密信息写入优化结果；不得把 `thinking` 直接广播给其他座位 |

#### 1.5.2 自进化闭环场景

1. 对局结束后，系统收集每个 Agent 的局内发言、行动、结果和胜负。
2. 离线分析任务对单座位表现打标签，例如“过早暴露身份”“遗漏关键信息”“发言与角色不一致”。
3. 生成下一版 Prompt 差异建议，并输出变更说明。
4. 经过审核后发布到该角色的 Prompt 版本库。
5. 下一局同角色 Agent 只读取最新已发布版本，不在运行时自改。
