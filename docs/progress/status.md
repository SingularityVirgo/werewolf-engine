...
细节规则见 PRD；包结构见 [code-modules](../reference/code-modules.md)；持久化见 **[ADR-007](../adr/007-persistence-redis-mysql.md)** · [persistence-rollout](../reference/persistence-rollout.md)（checklist）。

---

## 结论（一句话）

**PRD v1.0.17 MVP 已签字**（[mvp-signoff-2026-05-27.md](mvp-signoff-2026-05-27.md)）：规则引擎 + Formal 协议 + Mock/LLM 整局 + Web UI 展示层 + **ADR-007** 持久化/断线重连/托管；Formal 冒烟与 Mock **100 局**压测通过。v1.1 起做 JWT、P-08、LLM 百局压测、非法用例扩充等。
...
| `gateway` / `room` | **P0 完成**（[ADR-005](../adr/005-gateway-formal-path.md) §13）；**ADR-007** MySQL/Redis 写穿 + 归档 ✅ |
...
**测试（2026-05-27，Docker MySQL + Redis + dev profile）**

| 项 | 结果 |
|----|------|
| `mvnw.cmd test` | **108** 测全过（含 G-08 `ActionLogDedupeTest`） |
| `scripts/formal/illegal_action_smoke.py` | ✅ 非法 VOTE → ACTION_ACK 失败、阶段不变 |
| `scripts/formal/reconnect_grace_smoke.py` | ✅ 2s 内重连 `reconnected=true` |
| `scripts/formal/reconnect_hosting_smoke.py` | ✅ 32s 后 AI 托管、局终无死锁 |
| `scripts/formal/redis_cleanup_smoke.py` | ✅ GAME_OVER 后 room Redis 键清零 |
| `scripts/formal/frontend_ws_smoke.py` | ✅ **8/8**（PHASE_SYNC / GAME_EVENT / GAME_OVER） |
| `scripts/formal/formal_path_smoke.py` | ✅ **8/8**（dev 关闭 countdown + 后台 scheduler） |
| `scripts/formal/load_test_formal.py` | ✅ **100/100** 局 → `target/reports/load-test-100games.json` |
| MySQL `game_record` | ✅ 123 条归档（`werewolf.persistence.*=true`） |
...
| **P0** | 非法操作不污染状态 | ✅ 单测 + `illegal_action_smoke.py`（Formal 可扩充） | A/B |
| **P1** | 真人 + AI 混合（S1） | 建房/分座/托管 ✅；混合调度产品化 v1.1 | B |
| **P1** | AI JSON 50 次解析率 > 95%，**6s** 超时、解析**最多重试 2 次**后 fallback | ✅ A-02 + `llm-timeout-seconds=6`、`max-llm-retries=2`（`AiProperties` + `AIServiceRetryTest`） | A |
| **P1** | 离开 / 30s 重连 / 掉线托管 | ✅ ADR-007 阶段 3 + Formal smokes | B |
...
| — | 离开房间 / 掉线标记 | P1 | **是**（WAITING 离开 ✅；对局 GRACE / AI_HOSTED ✅） |
...
| — | 断线重连 30s | ✅ P-07 + `reconnect_grace_smoke.py` |
| — | 连接心跳 | ❌ |
| — | 独立 `GAME_OVER` WS 消息 | ✅ |
...
| 1 | MySQL `room` / `room_player` | ✅ 代码已落地 |
| 2 | `game_record` + action_log 归档 | ✅ 代码已落地 |
| 3 | Redis 会话 / 断线重连 | ✅ 代码已落地 |
| P1 | JWT / 用户表 | ❌ |
...
| 100 局无状态异常 | 压测 | ✅ Mock 100/100（`load-test-100games.json`）；LLM 百局 v1.1 |
...
| 里程碑 | 内容 | 状态 |
|--------|------|------|
| §8.1 D5-6 | 压测 20 局 | ✅（含于 100 局报告） |
| §8.3 D4-5 | 全角色 AI + **100 局压测报告** | ✅ Mock 100/100；LLM 单局/A-02 ✅ |
| §8.3 D6-7 | 真人 + Bot 混合局 | ✅ Formal 标准：`hostUserId` + 11 AI + smokes |
| §8.4 Day7 | 协议/Schema 冻结会 | 文档 ✅；前端 payload 微调 v1.1 |
...
| ID | 项 | 优先级 | 状态 |
|----|-----|--------|------|
| **F-13** | 狼夜 `KILL` / `ACTION_ACK` 反馈（PRD §4.8 **S-UI-04**） | **P0** | **✅**（2026-05-27） |
| — | 多人同房 UI | P1 | 未做 |
| — | 加入他人房间 UI | P1 | 未做 |
...
| G-04 | `action_log` MySQL 持久化 | §4.7 | **阶段 2 已实现**（`GameArchiveService`；需 `mysql-room` + `game-archive`） |
| G-06 | 狼夜超时随机刀（SM 兜底） | §4.3.3 | ✅ |
| — | 阶段超时兜底（与 P-05 联动） | §4.3.3、§4.3.7 | ✅ |
| — | LLM 6s + JSON 解析重试（`max-llm-retries=2`） | §4.5.4 / §5 | ✅ `AIServiceRetryTest` |
| **G-08** | `action_log` 单写 + 归档去重 | §4.7.3 | ✅ `GameActionRecorder` + `dedupeForArchive` |
...
| P-07 | Redis 会话 / token 鉴权 | §4.7.2、§2.3 | **阶段 3 已实现**（`TokenStore`、`SessionStore`、30s grace、`AI_HOSTED` 托管） |
| P-08 | 推送收窄 `affectedSeats` | ADR-005 | 未做 |
| R-02 | `join` 自动分座 | §4.2 | **完成** |
| — | `aiCount` 建房 | §4.2、S1 | **完成** |
| R-05 | 房间解散 | §6.1 | **完成** |
| — | 断线重连 30s、心跳 | §2.3、checklist G-07 | 重连 ✅；心跳 ❌ |
...
| F-13 | 狼夜击杀 / `ACTION_ACK` 反馈 | §4.8 S-UI-04 | **是**（2026-05-27） |
| — | 压测报告 v0.1（**100 局**，Formal 路径） | §8.3 | ✅ `target/reports/load-test-100games.json` |
| — | 压测 20 局（Week1 D5-6） | §8.1 | ✅ |
| — | `--full-bot` 12 座纯 Bot 实验路径 | — | 可选 |
| — | Formal 非法 WS 用例集 | §1.2 P0 | ✅ `illegal_action_smoke.py` |
| — | 压测 20/100 局 | §8.1/§8.3 | ✅ **100/100** `load_test_formal.py` |
| — | reconnect grace smoke | ADR-007 §7.3 | ✅ 2s 重连 + **32s 托管**（`reconnect_hosting_smoke.py`） |
| — | GAME_OVER Redis 清理 | ADR-007 §7.3 | ✅ `redis_cleanup_smoke.py` |
...
| 非法操作不污染状态 | **是**（单测） | **是**（`illegal_action_smoke.py`） |
| 12 Bot 同房不串房 | N/A | **是**（Day4） |
| 全角色 LLM 95% 解析 | 单测 + 整局验收 | **是**（A-02 报告） |
| 100 局压测无异常 | — | **是**（Mock 100/100） |
...
1. **P-08**：推送收窄 `affectedSeats`
2. **JWT** / 正式用户体系（替换 dev numeric token）
3. **LLM 100 局**压测报告（Mock 百局已完成）
4. Formal **非法 WS 用例集**扩充
5. **UX-01** 发牌体验（产品评审后）
...
**日期**：2026-05-27 · **基线**：PRD v1.0.17（规则/协议冻结不变）

→ [mvp-signoff-2026-05-27.md](mvp-signoff-2026-05-27.md)
...
| v2.6 | 2026-05-27 | **F-13** ✅；存储层升格 **[ADR-007](../adr/007-persistence-redis-mysql.md)** |
| v3.0 | 2026-05-27 | **MVP 签字**：PRD §8.5 + [mvp-signoff-2026-05-27.md](mvp-signoff-2026-05-27.md) |
| v2.10 | 2026-05-27 | ADR-007 §7.3 补测：托管、Redis 清理、countdown、frontend 8/8 |
