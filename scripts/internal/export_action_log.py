#!/usr/bin/env python3
"""
Internal 路径 A：导出内存 action_log（可选先跑 mock-auto-play）。

路径: scripts/internal/export_action_log.py
作用: 拉取 /internal/game action-log，写入 scripts/action_logs/；支持校验与时间线打印。

用法:
  python scripts/internal/export_action_log.py --play
  python scripts/internal/export_action_log.py --room-id <id> --validate --print
"""
...
OUT_DIR = Path(__file__).resolve().parent.parent / "action_logs"
